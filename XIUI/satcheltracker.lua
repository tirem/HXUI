...
local lastSatchelTextColor;

local satchelTracker = {};
...
--]]
satchelTracker.DrawWindow = function(settings)
	...
	local usedBagSlots = inventory:GetContainerCount(CONTAINER_SATCHEL);
	local maxBagSlots = inventory:GetContainerCountMax(CONTAINER_SATCHEL);
    ...
    if (imgui.Begin('SatchelTracker', true, windowFlags)) then

		imgui.Dummy({winSizeX,winSizeY});
		local locX, locY = imgui.GetWindowPos();

		-- Get custom colors
		local emptyColor = gConfig.colorCustomization.satchelTracker.emptySlotColor;

		-- Determine used slot color based on satchel usage thresholds
		local usedColor;
		if (usedBagSlots >= gConfig.satchelTrackerColorThreshold2) then
			usedColor = gConfig.colorCustomization.satchelTracker.usedSlotColorThreshold2;
		elseif (usedBagSlots >= gConfig.satchelTrackerColorThreshold1) then
			usedColor = gConfig.colorCustomization.satchelTracker.usedSlotColorThreshold1;
		else
			usedColor = gConfig.colorCustomization.satchelTracker.usedSlotColor;
		end
        ...
        if (settings.showText) then
			...
			satchelText:set_font_height(settings.font_settings.font_height);

            satchelText:set_text(usedBagSlots.. '/'..maxBagSlots);
            local textWidth, textHeight = satchelText:get_text_size();

            -- Calculate the actual right edge of the satchel dots (without window padding)
            local satchelWidth = (groupOffsetX * totalGroups) - settings.groupSpacing + settings.dotRadius;

            -- Position text at right edge of dots (text is right-aligned, so X position is the right edge)
            satchelText:set_position_x(locX + framePaddingX + satchelWidth);
		    satchelText:set_position_y(locY + settings.textOffsetY - textHeight);
		    -- Only call set_font_color if the color has changed (expensive operation for GDI fonts)
		    if (lastSatchelTextColor ~= gConfig.colorCustomization.satchelTracker.textColor) then
			    satchelText:set_font_color(gConfig.colorCustomization.satchelTracker.textColor);
			    lastSatchelTextColor = gConfig.colorCustomization.satchelTracker.textColor;
		    end
...
satchelTracker.Initialize = function(settings)
	-- Use FontManager for cleaner font creation
    satchelText = FontManager.create(settings.font_settings);
	allFonts = {satchelText};
end

satchelTracker.UpdateVisuals = function(settings)
	-- Use FontManager for cleaner font recreation
	satchelText = FontManager.recreate(satchelText, settings.font_settings);
	allFonts = {satchelText};

	-- Reset cached color when font is recreated
	lastSatchelTextColor = nil;
end


satchelTracker.SetHidden = function(hidden)
	if (hidden == true) then
		SetFontsVisible(allFonts, false);
	end
end

satchelTracker.Cleanup = function()
	-- Use FontManager for cleaner font destruction
	satchelText = FontManager.destroy(satchelText);
	allFonts = nil;
end

return satchelTracker;
